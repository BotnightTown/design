...
export default function Reason({number, title, children}: ReasonType){
  return(
    <div className="w-full xl:w-300 flex flex-col lg:flex-row items-center lg:justify-between gap-5">
      <div className="flex flex-row items-center gap-2 lg:gap-10">
        <div className="relative">
          <svg className="w-15 h-13.5 lg:w-20 lg:h-18" viewBox="0 0 78 73" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <p className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-main font-semibold text-[16px] lg:text-[24px] text-[#55CE9C]">{number}.</p>
        </div>
        <H3>{title}</H3>
      </div>
      {children}
